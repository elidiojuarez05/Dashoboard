        ...
        if not problemas: 
            ...
            for inicio in range(0,len(problemas),2):
                ...
                for j,(m,est) in enumerate(problemas[inicio:inicio+2]):
                    with cols[j]:
                        render_html(f'<div class="machine-card"><div class="machine-head"><div><div class="machine-name">⚠️ {ui_escape(m)}</div><div class="machine-type">{"ROUTER" if es_maquina_router(m) else "IMPRESIÓN"}</div></div><div class="machine-pill" style="background:#7f1d1d;color:#fca5a5;border-color:#991b1b;">⚠️ {ui_escape(est).upper()}</div></div><div class="machine-body"><div class="machine-empty"><div style="font-size:32px">🛠️</div><strong>ESTACIÓN BLOQUEADA</strong><div>Inspeccionar bitácora de mantenimiento o falla mecánica.</div></div></div></div>')
